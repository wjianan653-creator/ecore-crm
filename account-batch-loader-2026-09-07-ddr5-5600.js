(() => {
  ...
  const DATA_URL = "./account-batch-2026-09-07-ddr5-5600-sourcing.json?v=1";
  const BATCH_KEY = "ecore-account-batch-2026-09-07-ddr5-5600-v1";
  ...
  function toast(text, error = false) {
    document.getElementById("ecore-ddr5-import-toast")?.remove();
    const el = document.createElement("div");
    el.id = "ecore-ddr5-import-toast";
    el.textContent = text;
    Object.assign(el.style, {
      position: "fixed", right: "18px", bottom: "72px", zIndex: 999999,
      maxWidth: "520px", padding: "13px 17px", borderRadius: "10px",
  ...
  async function addOne(c) {
    ...
    setField(form, "products", c.products || "32GB DDR5-5600 ECC RDIMM");
    setField(form, "commercialHypothesis", c.commercialHypothesis || "Qualified sourcing account for 32GB DDR5-5600 ECC RDIMM; verify physical stock before deeper investment.");
    setField(form, "verifiedEvidence", c.verifiedEvidence || "2026-09-07 researched sourcing candidate; physical stock not assumed.");
    setField(form, "nextAction", c.nextAction || "Verify exact PN, physical qty, price, condition and stock location.");
    setField(form, "notes", c.notes || `2026-09-07 DDR5-5600 sourcing pool｜${c.priority || "B1"}｜尚未触达`);
  ...
  async function importBatch(button) {
    ...
    if (button) {
      ...
      button.textContent = "正在导入32G 5600渠道…";
    }

    try {
      if (!(await goClients())) throw new Error("无法打开账户库");
      const response = await fetch(DATA_URL, { cache: "no-store" });
      if (!response.ok) throw new Error(`32G 5600名单文件加载失败 ${response.status}`);
      ...
      toast(`32G 5600渠道导入完成：新增 ${added} 家，跳过 ${skipped} 个已有账户。`);
      if (button) button.textContent = `32G 5600已导入：+${added} / 跳过${skipped}`;
    } catch (err) {
      importStarted = false;
      toast(`32G 5600渠道导入未完成：${err.message || err}`, true);
      if (button) {
        button.disabled = false;
        button.textContent = "重试导入32G 5600渠道10家";
      }
    }
  }
  ...
  function addImportButton() {
    ...
    if (document.getElementById("import-ddr5-5600-accounts-20260907")) return;
    const host = document.querySelector(".sidebar") || document.body;
    const button = document.createElement("button");
    button.id = "import-ddr5-5600-accounts-20260907";
    button.type = "button";
    button.textContent = "＋ 导入32G 5600渠道10家";
  ...
  function maybeAutoImport() {
    ...
    if (!document.querySelector('[data-view="clients"]')) return;
    autoAttempted = true;
    setTimeout(() => importBatch(document.getElementById("import-ddr5-5600-accounts-20260907")), 1000);
  }
