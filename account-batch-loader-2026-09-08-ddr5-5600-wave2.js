(() => {
  ...
  const DATA_URL = "./account-batch-2026-09-08-ddr5-5600-wave2.json?v=1";
  const BATCH_KEY = "ecore-account-batch-2026-09-08-ddr5-5600-wave2-v1";
  ...
  function toast(text, error = false) {
    document.getElementById("ecore-ddr5-wave2-import-toast")?.remove();
    const el = document.createElement("div");
    el.id = "ecore-ddr5-wave2-import-toast";
    el.textContent = text;
  ...
  async function addOne(c) {
    ...
    setField(form, "commercialHypothesis", c.commercialHypothesis || "Qualified sourcing account; verify physical stock before deeper investment.");
    setField(form, "verifiedEvidence", c.verifiedEvidence || "2026-09-08 researched sourcing candidate; physical stock not assumed.");
    setField(form, "nextAction", c.nextAction || "Verify availability first, then exact PN, physical qty, price, stock location and HK export capability.");
    setField(form, "notes", c.notes || `2026-09-08 DDR5-5600 sourcing pool｜${c.priority || "B1"}｜尚未触达`);
  ...
  async function importBatch(button) {
    ...
    if (button) {
      ...
      button.textContent = "正在导入32G 5600第二批…";
    }
    ...
    try {
      ...
      toast(`32G 5600第二批导入完成：新增 ${added} 家，跳过 ${skipped} 个已有账户。未实际触达的账户会保持未触达状态。`);
      if (button) button.textContent = `32G 5600第二批已导入：+${added} / 跳过${skipped}`;
    } catch (err) {
      importStarted = false;
      toast(`32G 5600第二批导入未完成：${err.message || err}`, true);
      if (button) {
        button.disabled = false;
        button.textContent = "重试导入32G 5600第二批9家";
      }
    }
  }
  ...
  function addImportButton() {
    ...
    if (document.getElementById("import-ddr5-5600-accounts-20260908-wave2")) return;
    const host = document.querySelector(".sidebar") || document.body;
    const button = document.createElement("button");
    button.id = "import-ddr5-5600-accounts-20260908-wave2";
    button.type = "button";
    button.textContent = "＋ 导入32G 5600第二批9家";
  ...
  function maybeAutoImport() {
    ...
    if (!document.querySelector('[data-view="clients"]')) return;
    autoAttempted = true;
    setTimeout(() => importBatch(document.getElementById("import-ddr5-5600-accounts-20260908-wave2")), 1000);
  }
